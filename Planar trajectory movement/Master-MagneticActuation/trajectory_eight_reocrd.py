...
if __name__ == '__main__':
    ...
    thread_receive = threading.Thread(target=my_serial_receive.receive_data,
                                      args=('COM9', 115200, 'data_sync\euler_eight-50Hz.TXT'))
    # 8字运动
    for i in range(1000):
        ...
        if i == 0:
            ...
            my_serial_receive.receive_flag = True
            thread_receive.start()

        if i == 11:
            my_serial_receive.receive_flag = False
            thread_receive.join()
        ...
        # print(c1_sel, c1_no, c2_sel, c2_no, c3_sel, c3_no)

        if c1_sel == 2 and c1_no == 8:
            attract_force = - attract_force
        if c2_sel == 2 and c2_no == 8:
            repell_force = - repell_force
